...
class Database {
  ...
  addProductToShoppingCart({firstName, productName}, callback = (error) => {}){
    this.connect((error, database) => {
      if (error) {
        callback(error);
      } else {
        ...
        database.collection(users).updateOne({"firstName":firstName},{$push:{"shoppingCart": productName}}, callback);
  ...
  addReviewToProduct( {productName, review}, callback = (messageResult) => {}){
    this.connect((error, database) => {
      if (error) {
        ...
        database.collection(products).updateOne({"productName":productName},{$push:{"reviews": review}}, callback);
